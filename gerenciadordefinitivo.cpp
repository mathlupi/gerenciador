...
void add_daily_task(DailyTask daily_tasks[], int *daily_task_count) {
    ...
    if (reminder_option == 1) {
        ...
        scanf("\n %d", &daily_tasks[*daily_task_count].interval_days);
        for (int i = 0; i < 7; i++) {
        ...
        printf("\n Opcao invalida. Não sera configurado lembrete especifico.");
        daily_tasks[*daily_task_count].interval_days = 0;
        for (int i = 0; i < 7; i++) {
            daily_tasks[*daily_task_count].specific_days[i] = 0;
        }
...
void list_daily_tasks(DailyTask daily_tasks[], int daily_task_count) {
    ...
    for (int i = 0; i < daily_task_count; i++) {
        printf("\n %d. %s [%s] - Hora Limite: %s", i + 1, daily_tasks[i].name,
               daily_tasks[i].is_completed ? "\n Concluida" : "\n Pendente",
               daily_tasks[i].due_time);
    }
}

void weekly_report(Task tasks[], int task_count) {
}

void monthly_report(Task tasks[], int task_count){
   
}

void remove_task(Task tasks[], int *task_count) {
    if (*task_count == 0) {
        printf("Nenhuma tarefa disponivel para remoçao.\n");
        return;
    }
...
void remove_daily_task(DailyTask daily_tasks[], int *daily_task_count) {
    if (*daily_task_count == 0) {
        printf("Nenhuma tarefa diaria disponivel para remoçao.\n");
        return;
    }
    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 8:
                ...
                break;
            case 9:
                weekly_report(tasks, task_count);
                break;
